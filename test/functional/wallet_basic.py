...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(self.nodes[0].getbalance(), 10000*100-21)
        ...
        assert_equal(self.nodes[2].getbalance(), 100*10000-6)
        assert_equal(self.nodes[2].getbalance("from1"), 100*10000-6-21)
        ...
        self.sync_all([self.nodes[0:3]])
        node_2_bal = self.check_fee_amount(self.nodes[2].getbalance(), Decimal('999984'), fee_per_byte, count_bytes(self.nodes[2].getrawtransaction(txid)))
        ...
        outputs = {self.nodes[1].getnewaddress(): 499999, self.nodes[0].getnewaddress(): 11.11}
        rawTx = self.nodes[1].createrawtransaction(inputs, outputs).replace("fcb10100", "00000000") #replace 11.11 with 0.0 (int32)
        ...
        txObj = self.nodes[0].gettransaction(txId)
        assert_equal(txObj['amount'], Decimal('-0.1'))
        ...
        assert_raises_rpc_error(-4, "Transaction has too long of a mempool chain", self.nodes[0].sendtoaddress, sending_addr, node0_balance - Decimal('0.1'))
