...
bool CWallet::SignTransaction(CMutableTransaction &tx)
{
    ...
    for (const auto& input : tx.vin) {
        ...
        if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, amount, SIGHASH_BCX_ALL), scriptPubKey, sigdata)) {
            return false;
        }
    ...
    unsigned int nBytes;
    {
                ...
                if (hasSender && coin_control.HasSelected()) {
                    for (size_t i = 0; i != vCoins.size(); i++) {
                        if (vCoins[i].outpoint == senderInput) {
                            if (i != 0) {
                                std::iter_swap(vCoins.begin(), vCoins.begin() + i);
                            }
                            break;
                        }
                    }
        ...
        if (sign)
        {
            ...
            for (const auto& coin : vCoins)
            {
                ...
                if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, coin.txout.nValue, SIGHASH_BCX_ALL), scriptPubKey, sigdata))
                {
                    strFailReason = _("Signing transaction failed");
                    return false;
                } else {
