...
double GetDifficultyBCX(const CBlockIndex* blockindex)
{
    ...
    uint32_t powLimit = UintToArith256(Params().GetConsensus().powLimitBCXStart).GetCompact();
...
double GetDifficultyBTC(const CBlockIndex* blockindex)
{
...
double GetDifficulty(const CBlockIndex* blockindex)
{
    ...
    if (blockindex->nHeight >= Params().GetConsensus().BCXHeight) {
        return GetDifficultyBCX(blockindex);
    } else {
        return GetDifficultyBTC(blockindex);
    }
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of bitcoinx addresses\n"
