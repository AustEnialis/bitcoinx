...
// Copyright (c) 2017-2018 The BitcoinX developers
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    {
        if ((pindexLast->nHeight + 1) < (params.BCXHeight + params.BCXPowLimitWindow)) {
           return UintToArith256(params.powLimitBCXStart).GetCompact();
        } else {
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    // Check range
    if (fNegative || bnTarget == 0 || fOverflow || bnTarget > UintToArith256(params.powLimitBCXStart))
        return false;
...
uint32_t  GetNextWorkRequiredByDAA(const CBlockIndex* pindexLast, const CBlockHeader* pblock, const Consensus::Params& params) {
    ...
    // Special difficulty rule for testnet:
    uint32_t nProofOfWorkLimit = UintToArith256(params.powLimitBCXStart).GetCompact();
    if (params.fPowAllowMinDifficultyBlocks) {
    ...
    arith_uint256 powLimit = UintToArith256(params.powLimitBCXStart);
